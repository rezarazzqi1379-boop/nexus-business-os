...
WorkDomain = Literal["research", "market_intelligence", "customer_network", "inbox_monitoring", "coding_learning", "backup", "knowledge_management", "innovation", "security", "news_monitoring"]
...
_ALLOWED_DOMAINS = {"research", "market_intelligence", "customer_network", "inbox_monitoring", "coding_learning", "backup", "knowledge_management", "innovation", "security", "news_monitoring"}
...
def _text_errors(name: str, value: object, max_length: int) -> list[str]:
    ...
    if any(category(ch) in _DISALLOWED_UNICODE_CATEGORIES for ch in value): errors.append(f"{name} cannot contain control or formatting characters")
    return errors
...
def _outcome_errors(task: WorkItem) -> list[str]:
    ...
    if all(value is None for value in values): return []
    if any(value is None for value in values): return ["outcome contract requires goal_ref, success_signal and failure_signal together"]
    errors: list[str] = []
...
def validate_work_item(task: WorkItem) -> list[str]:
    ...
    errors: list[str] = []
    errors.extend(_text_errors("task_id", task.task_id, _MAX_META_LENGTH)); errors.extend(_text_errors("objective", task.objective, _MAX_OBJECTIVE_LENGTH))
    if not isinstance(task.domain, str) or task.domain not in _ALLOWED_DOMAINS: errors.append("domain must be supported")
    if not isinstance(task.action_kind, str): errors.append("action_kind must be a string")
    elif task.action_kind not in _ALLOWED_ACTION_KINDS: errors.append("action_kind must be supported")
    ...
    errors.extend(_ref_errors("evidence_refs", task.evidence_refs)); errors.extend(_outcome_errors(task))
    if not isinstance(task.acceptable_capability_ids, tuple): errors.append("acceptable_capability_ids must be a tuple")
    elif not task.acceptable_capability_ids: errors.append("acceptable_capability_ids requires at least one capability")
    else:
        seen: set[str] = set()
...
def _safe_rank(mapping: dict[str, int], value: object) -> int:
    return mapping.get(value, _INVALID_RANK) if isinstance(value, str) else _INVALID_RANK

def _priority_key(task: WorkItem) -> tuple[object, ...]:
    if not isinstance(task, WorkItem): return (_INVALID_RANK, _INVALID_RANK, _INVALID_RANK, _INVALID_RANK, "~invalid-work-item")
    task_id = task.task_id if isinstance(task.task_id, str) else "~invalid-task-id"
    return (_safe_rank(_PRIORITY_RANK, task.value), _safe_rank(_PRIORITY_RANK, task.urgency), _safe_rank(_EVIDENCE_RANK, task.evidence), _safe_rank(_COST_RANK, task.cost), task_id)
...
def plan_autonomy(tasks: Sequence[WorkItem], capabilities: Sequence[Capability]) -> AutonomyPlan:
    ...
    for task in sorted(tasks, key=_priority_key):
        ...
        if validation_errors:
            planned.append(PlannedWork(task, "blocked", (), tuple(validation_errors), _invalid_gate())); continue
        ...
        selected_ids = tuple(cap.capability_id for cap in capability_plan.selected); blockers = list(capability_plan.unresolved_need_ids)
        if blockers: status: PlanStatus = "blocked"
            ...
            status = "human_gate"; blockers.extend(f"capability approval required: {capability_id}" for capability_id in capability_plan.approval_required_capability_ids)
