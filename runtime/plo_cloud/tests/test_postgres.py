...
def test_stale_approval_version_blocked():
    s = fresh()
    rid = s.enqueue("send", "stale-key", True)
    aid = s.request_approval(rid, "send:a")
    c = s.claim_next("A")
    s.decide_approval(aid, "approved")
    try:
...
def test_exact_scope_approval_before_intent():
    s = fresh()
    rid = s.enqueue("send", "approve-key", True)
    c = s.claim_next("A")
    aid = s.request_approval(rid, "send:a")
    s.decide_approval(aid, "approved")
    try:
    ...
    s.record_intent(rid, "op-good", tok)


def test_approval_single_use():
    s = fresh()
    rid = s.enqueue("send", "single-use", True)
    c = s.claim_next("A")
    aid = s.request_approval(rid, "send:a")
    s.decide_approval(aid, "approved")
...
def test_orphan_recovery_and_fencing():
    s = fresh()
    rid = s.enqueue("task", "orphan")
    old = s.claim_next("A", -1)
    recovered = s.recover_orphans()
    assert rid in recovered
    new = s.claim_next("B", 30)
    assert new and new["run_id"] == rid
    try:
...
def test_duplicate_execution_metric():
    s = fresh()
    rid = s.enqueue("send", "dup", True)
    c = s.claim_next("A")
    aid = s.request_approval(rid, "send:a")
    s.decide_approval(aid, "approved")
    tok = s.authorize_operation(rid, "op-dup", "send:a", c["_lease_token"], "A")
    s.record_intent(rid, "op-dup", tok)
    s.mark_executed("op-dup")
    s.mark_executed("op-dup")
