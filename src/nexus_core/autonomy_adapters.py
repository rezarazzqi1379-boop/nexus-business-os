...
def inbox_reply_work_item(*, message_ref: str, thread_key: str, decision_relevant: bool, external_reply_needed: bool) -> WorkItem:
    if external_reply_needed:
        return WorkItem(
            ...
            "Review a decision-relevant inbox reply and prepare the next external response.",
            ...
            success_signal="A verified blocker is resolved or a human-reviewed next response is ready with no invented facts.",
            failure_signal="The thread remains blocked because required engineering/commercial evidence is still missing or the reply would rely on an unsupported claim.",
        )
    return WorkItem(
        f"inbox-monitor:{thread_key}", "inbox_monitoring",
        "Review the inbox signal and update internal project state if it changes a decision.",
        "read", ("gmail.read",), (message_ref,),
        "high" if decision_relevant else "low", "high" if decision_relevant else "low", "strong", "low", False, True,
        goal_ref=f"goal:commercial-thread:{thread_key}",
        success_signal="A decision-relevant state change is identified and linked to retrievable evidence.",
        failure_signal="No material state change is found; do not generate follow-up work merely for activity.",
    )
...
def learning_signal_work_item(*, source_ref: str, topic_key: str, implementation_relevant: bool) -> WorkItem:
    return WorkItem(
        ...
        "Study the authoritative technical source, extract only implementation-relevant techniques, compare them with the current NEXUS architecture, and propose a testable code or review change if justified.",
