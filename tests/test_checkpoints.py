...
def test_same_idempotency_key_same_source_version_and_same_content_is_a_match():
    assert checkpoint_matches(manifest(captured_at="2026-08-19T17:00:00+00:00"), manifest()) is True

def test_changed_content_is_detected_for_incremental_backup():
    assert changed_artifact_refs(manifest(digest=HASH_B), manifest(digest=HASH_A)) == ("notion:command-center",)
...
def test_duplicate_artifact_refs_are_rejected():
    duplicate = CheckpointManifest("checkpoint:test", "test", "2026-08-19T16:00:00+00:00", "backup:test:v1", (
...
def test_stale_receipt_version_does_not_prove_fresh_backup_even_when_digest_matches():
    assert receipt_covers_manifest_entry(receipt(source_version_ref=VERSION_A), manifest(source_version_ref=VERSION_B)) is False

def test_receipt_requires_timezone_aware_storage_time():
    assert "stored_at must include a timezone offset" in validate_backup_receipt(receipt(stored_at="2026-08-19T19:44:00"))
