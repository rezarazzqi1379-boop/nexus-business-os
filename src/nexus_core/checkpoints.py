...
SnapshotCategory = Literal[
    "code", "decision", "evidence", "research", "commercial", "configuration", "documentation",
]
_ALLOWED_CATEGORIES = {"code", "decision", "evidence", "research", "commercial", "configuration", "documentation"}
...
def _meta_error(name: str, value: object) -> str | None:
    ...
    if len(value) > _MAX_META_LENGTH: return f"{name} must be at most {_MAX_META_LENGTH} characters"
...
def _sha256_error(value: object) -> str | None:
    if not isinstance(value, str): return "content_sha256 must be a string"
    if len(value) != 64 or any(ch not in "0123456789abcdef" for ch in value): return "content_sha256 must be a lowercase 64-character SHA-256 hex digest"
    return None
...
def _timezone_error(name: str, value: object) -> str | None:
    ...
    if meta_error: return meta_error
    assert isinstance(value, str)
    try: parsed = _parse_timestamp(value)
...
def validate_checkpoint(manifest: CheckpointManifest) -> list[str]:
    if not isinstance(manifest, CheckpointManifest): return ["manifest must be a CheckpointManifest"]
    errors: list[str] = []
    ...
    if captured_error: errors.append(captured_error)
    if not isinstance(manifest.entries, tuple): return errors + ["entries must be a tuple"]
    ...
    for entry in manifest.entries:
        ...
        for name, value in (("artifact_ref", entry.artifact_ref), ("source_ref", entry.source_ref), ("source_version_ref", entry.source_version_ref)):
            error = _meta_error(name, value)
            if error: errors.append(error)
        if not isinstance(entry.category, str) or entry.category not in _ALLOWED_CATEGORIES: errors.append("snapshot category must be supported")
        digest_error = _sha256_error(entry.content_sha256)
        if digest_error: errors.append(digest_error)
        if isinstance(entry.artifact_ref, str):
            if entry.artifact_ref in seen_artifacts: errors.append("checkpoint cannot contain duplicate artifact_ref values")
            seen_artifacts.add(entry.artifact_ref)
    return errors


def validate_backup_receipt(receipt: BackupReceipt) -> list[str]:
    if not isinstance(receipt, BackupReceipt): return ["receipt must be a BackupReceipt"]
    errors: list[str] = []
    ...
    if stored_at_error: errors.append(stored_at_error)
    digest_error = _sha256_error(receipt.source_content_sha256)
    if digest_error: errors.append(digest_error.replace("content_sha256", "source_content_sha256"))
    return errors


def validate_restore_proof(proof: RestoreProof) -> list[str]:
    if not isinstance(proof, RestoreProof): return ["proof must be a RestoreProof"]
    errors: list[str] = []
    ...
    if restored_at_error: errors.append(restored_at_error)
    for name, value in (("expected_content_sha256", proof.expected_content_sha256), ("restored_content_sha256", proof.restored_content_sha256)):
        ...
        if digest_error: errors.append(digest_error.replace("content_sha256", name))
    return errors


def receipt_covers_manifest_entry(receipt: BackupReceipt, manifest: CheckpointManifest) -> bool:
    if validate_backup_receipt(receipt) or validate_checkpoint(manifest): return False
    if receipt.checkpoint_id != manifest.checkpoint_id: return False
    if _parse_timestamp(receipt.stored_at) < _parse_timestamp(manifest.captured_at): return False
...
def restore_proves_reconstruction(proof: RestoreProof, receipt: BackupReceipt, manifest: CheckpointManifest) -> bool:
    if validate_restore_proof(proof): return False
    if not receipt_covers_manifest_entry(receipt, manifest): return False
    if _parse_timestamp(proof.restored_at) < _parse_timestamp(receipt.stored_at): return False
    if proof.checkpoint_id != receipt.checkpoint_id or proof.artifact_ref != receipt.artifact_ref or proof.backend != receipt.backend or proof.stored_artifact_ref != receipt.stored_artifact_ref or proof.source_version_ref != receipt.source_version_ref or proof.expected_content_sha256 != receipt.source_content_sha256: return False
...
def checkpoint_matches(manifest: CheckpointManifest, previous: CheckpointManifest | None) -> bool:
    ...
    if validate_checkpoint(manifest) or validate_checkpoint(previous): return False
    if manifest.idempotency_key != previous.idempotency_key: return False
...
def changed_artifact_refs(manifest: CheckpointManifest, previous: CheckpointManifest | None) -> tuple[str, ...] | None:
    if validate_checkpoint(manifest): return None
    previous_by_ref = {}
