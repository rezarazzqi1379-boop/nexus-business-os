...
class TraceEvent:
    """Stable internal NEXUS observability envelope.

    The canonical envelope stores metadata and retrievable references, never raw
    commercial/model/tool payloads. Exporters may later map this stable contract
    to OpenAI tracing, OpenTelemetry, or another backend without changing the
    ...
    def to_dict(self) -> dict[str, object]:
        """Return stable serialization without vendor-specific telemetry names."""
...
def validate_trace(events: Sequence[TraceEvent]) -> TraceValidationResult:
    """Validate one reconstructable trace graph and fail closed on ambiguity."""
    ...
    for event in events:
        errors.extend(
            f"event[{event.event_id or '?'}]: {error}" for error in event.validate()
        )
    ...
    terminals = tuple(
        event.event_id for event in events if not children_by_parent.get(event.event_id)
    )
