...
def validate_audit_trace(events: Sequence[AuditEvent]) -> AuditTraceValidation:
    ...
    for event in events:
        ...
        if parent is None:
            errors.append(f"event[{event.event_id}]: parent_event_id must reference an event in the trace set")
            continue
        if parent.trace_id != event.trace_id:
            errors.append(f"event[{event.event_id}]: parent_event_id must stay within the same trace_id")
    for event in events:
        seen: set[str] = set()
        current = event
