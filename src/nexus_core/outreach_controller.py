...
@dataclass(frozen=True)
class OutreachBatch:
    """Immutable reviewed outreach plan.

    Follow-ups may be planned and content-bound in the batch, but they are not
    pre-authorized by a batch approval. Actual future sends require their own exact
    action-specific approval at execution time.
    """
...
def validate_outreach_batch(batch: OutreachBatch) -> tuple[str, ...]:
    ...
    errors: list[str] = []
    if not _valid_text(batch.batch_id):
        errors.append("batch_id is invalid")
    if not _valid_text(batch.campaign_ref):
        errors.append("campaign_ref is invalid")
    if not _valid_refs(batch.source_version_refs):
    ...
    for target in batch.targets if isinstance(batch.targets, tuple) else ():
        if not isinstance(target, OutreachTarget):
            ...
            continue
        if not _valid_text(target.target_id):
            errors.append("target_id is invalid")
        if target.target_id in target_ids:
            errors.append("duplicate target_id")
        target_ids.add(target.target_id)
        if not _valid_text(target.company_name):
            errors.append("company_name is invalid")
        if not _valid_text(target.recipient):
            errors.append("recipient is invalid")
        if not _valid_text(target.role):
            errors.append("role is invalid")
        if target.channel not in ("email", "linkedin", "whatsapp", "other"):
            errors.append("channel is invalid")
        recipient_key = (
            target.channel,
            target.recipient.casefold() if isinstance(target.recipient, str) else "",
        )
        if recipient_key in recipients:
            errors.append("duplicate recipient/channel pair")
        recipients.add(recipient_key)
        if not _valid_refs(target.company_fit_refs):
            errors.append("company_fit_refs are invalid")
        if not _valid_refs(target.need_evidence_refs):
            errors.append("need_evidence_refs are invalid")
        if not _valid_refs(target.contact_path_refs):
            errors.append("contact_path_refs are invalid")
    ...
    for message in batch.messages if isinstance(batch.messages, tuple) else ():
        if not isinstance(message, PreparedMessage):
            ...
            continue
        if not _valid_text(message.target_id):
            errors.append("message target_id is invalid")
        if message.target_id in message_target_ids:
            errors.append("duplicate message target_id")
        message_target_ids.add(message.target_id)
        if not _valid_text(message.subject):
            errors.append("subject is invalid")
        if not _valid_text(message.body, max_length=_MAX_BODY):
            errors.append("body is invalid")
    ...
    for followup in batch.followups if isinstance(batch.followups, tuple) else ():
        ...
        if not _valid_text(followup.subject):
            errors.append("followup subject is invalid")
        if not _valid_text(followup.body, max_length=_MAX_BODY):
            errors.append("followup body is invalid")
        ...
        if isinstance(followup.step, int) and not isinstance(followup.step, bool):
            ...
            if key in followup_keys:
                errors.append("duplicate followup target/step")
        ...
        "targets": [
            {
                ...
                "company_fit_refs": list(target.company_fit_refs),
                "need_evidence_refs": list(target.need_evidence_refs),
                "contact_path_refs": list(target.contact_path_refs),
            }
        ...
        "followups": [
            {
                ...
                "step": followup.step,
                "wait_after_hours": followup.wait_after_hours,
                "subject": followup.subject,
                "body": followup.body,
                "stop_on_reply": followup.stop_on_reply,
            }
            for followup in batch.followups
        ],
    }
    encoded = json.dumps(
        payload,
...
def preview_outreach_release(batch: OutreachBatch) -> OutreachRelease:
    if not isinstance(batch, OutreachBatch):
        return OutreachRelease(
            "", "", "", GateDecision(False, False, "batch must be an OutreachBatch"), ()
        )
    ...
    if errors:
        return OutreachRelease(
            ...
            digest,
            action_id,
            GateDecision(False, False, "; ".join(errors)),
            (),
        )
    ...
    gate = evaluate_action(
        ActionIntent(
            ...
            description=f"Release prepared initial outreach messages for batch {batch.batch_id}",
            reversible=False,
        )
    )
...
def authorize_outreach_release(
    ...
    *,
    ...
    """Authorize only the initial messages in the reviewed batch.

    Planned follow-ups remain part of the immutable review context but are never
    pre-authorized here. They require a separate exact-send action at execution time.
    """
    ...
    if current >= expires_at:
        return OutreachRelease(
            ...
            digest,
            action_id,
            GateDecision(False, True, "outreach batch approval envelope expired"),
            (),
        )
    ...
    approved_initials = tuple(f"{message.target_id}:initial" for message in batch.messages)
    return OutreachRelease(batch.batch_id, digest, action_id, gate, approved_initials)
