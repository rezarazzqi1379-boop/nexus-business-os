...
def validate_goal_track(goal: GoalTrack) -> list[str]:
    if not isinstance(goal, GoalTrack):
        return ["goal must be a GoalTrack"]
    errors: list[str] = []
    for name, value, max_len in (
        ("goal_ref", goal.goal_ref, _MAX_META),
        ("objective", goal.objective, _MAX_TEXT),
        ("success_signal", goal.success_signal, _MAX_TEXT),
        ("failure_signal", goal.failure_signal, _MAX_TEXT),
    ):
        error = _text_error(name, value, max_len)
        if error:
            errors.append(error)
    if not isinstance(goal.horizon, str) or goal.horizon not in _ALLOWED_HORIZONS:
        errors.append("horizon must be supported")
    if not isinstance(goal.state, str) or goal.state not in _ALLOWED_STATES:
        errors.append("state must be supported")
    for name, value in (
        ("next_action_ref", goal.next_action_ref),
        ("blocker_ref", goal.blocker_ref),
        ("pause_reason", goal.pause_reason),
        ("last_outcome_ref", goal.last_outcome_ref),
    ):
    ...
    if goal.state == "next_action":
        if goal.next_action_ref is None:
            errors.append("next_action state requires next_action_ref")
        if any(value is not None for value in (goal.blocker_ref, goal.review_at, goal.pause_reason)):
            errors.append("next_action state cannot carry blocker/review/pause routing")
    elif goal.state == "waiting_blocked":
        if goal.blocker_ref is None:
            errors.append("waiting_blocked state requires blocker_ref")
        if any(value is not None for value in (goal.next_action_ref, goal.review_at, goal.pause_reason)):
            errors.append("waiting_blocked state cannot carry next-action/review/pause routing")
    elif goal.state == "scheduled_review":
        if goal.review_at is None:
            errors.append("scheduled_review state requires review_at")
        if any(value is not None for value in (goal.next_action_ref, goal.blocker_ref, goal.pause_reason)):
            errors.append("scheduled_review state cannot carry next-action/blocker/pause routing")
    elif goal.state == "explicit_pause":
        if goal.pause_reason is None:
            errors.append("explicit_pause state requires pause_reason")
        if any(value is not None for value in (goal.next_action_ref, goal.blocker_ref, goal.review_at)):
            errors.append("explicit_pause state cannot carry next-action/blocker/review routing")
    return errors


def route_goal_portfolio(goals: Sequence[GoalTrack]) -> GoalPortfolio:
    ready: list[GoalTrack] = []
    blocked: list[GoalTrack] = []
