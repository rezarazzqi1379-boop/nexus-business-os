...
from exact_external_gate import ExactExternalMessage
...
def build_exact_message_from_reviewed_payload(
    ...
    *,
    ...
    """Build an exact send snapshot from trusted reviewed fields.

    Raw external content is never copied into any control-plane authorization field. The
    body used for a potential send must be supplied separately as reviewed_body.
    """
    if not isinstance(ingress, ExternalMessageIngress):
        raise ValueError("ingress must be an ExternalMessageIngress")
