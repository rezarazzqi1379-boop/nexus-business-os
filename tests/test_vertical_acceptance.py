...
def _run(run_id: str, *, corrections: int = 0, unknowns: int = 1, blocked: int = 1, duplicates: int = 1, prevented: int = 1, time_ms: int = 900, project_id: str = "PRJ-HYD-01", candidate_id: str = "hydro-v1", external_effects: int = 0, leaks: int = 0, violations: int = 0, sensitive: bool = False) -> VerticalRunObservation:
...
def test_cross_project_contamination_fails_closed():
    result = assess_vertical_runs(
        "PRJ-HYD-01",
        "hydro-v1",
        [_run("r1"), _run("r2", project_id="PRJ-CAN-01")],
        _policy(),
    )
...
def test_external_effect_or_policy_violation_cannot_pass_eval():
    result = assess_vertical_runs(
        "PRJ-HYD-01",
        "hydro-v1",
        [_run("r1"), _run("r2", external_effects=1, violations=1)],
        _policy(),
    )
    assert result.verdict == "FAIL"
    assert any("external effects" in reason for reason in result.reasons)
    assert any("policy violation" in reason for reason in result.reasons)


def test_sensitive_trace_payload_is_rejected():
...
def test_duplicate_run_id_is_not_double_counted_as_valid_evidence():
    result = assess_vertical_runs("PRJ-HYD-01", "hydro-v1", [_run("r1"), _run("r1")], _policy())
    assert result.verdict == "FAIL"
